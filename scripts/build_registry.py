...
Layout open-siope:
- dataset.yml in ``datasets/*/`` (entrate/uscite) e ``support/*/`` (anagrafiche seed);
- parquet e run records in ``out/data/`` (root dichiarato nei dataset.yml);
- GCS: ``gs://dataciviclab-{clean,mart}/siope/{slug}/{year}/`` (layout year).
...
def main() -> int:
    ...
    contract = PathContract(prefix="siope", clean_layout="year", mart_layout="year")
